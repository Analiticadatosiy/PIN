...
if data_file is not None and ejecucion is not None :
    ...
    df_p2 = pd.read_excel(data_file, sheet_name='Hoja2')
    Unidades_Producidas = np.array(df_p2.loc[:, 'Unidades_Producidas'])
    Fecha_unid=np.array(df_p2.loc[:, 'Fecha'])
    ...
    df_p=pd.read_excel(data_file,sheet_name='Hoja1')
    tamano = len(df_p.index)
    #df_p3 = pd.read_excel("referenciasNMAX_modificado.xlsx", sheet_name='Hoja3')
    df_p3=pd.read_excel(data_file,sheet_name='Hoja3')
    Fecha_etapa_ant = np.array(df_p3.loc[:, 'Fecha_etapa_ant'])
    Valor_unitario = np.array(df_p3.loc[:, 'Valor_unitario'])
    Fecha_comp_imp = np.array(df_p3.loc[:, 'Fecha_comp_imp'])
    Componentes_importados = np.array(df_p3.loc[:, 'Componentes_importados'])

    
    
    lista = list(product([0, 1], repeat=tamano))
    Precio_CKD = np.array(df_p.loc[:, 'Precio_CKD'])
    Precio_local = np.array(df_p.loc[:, 'Precio_local'])
    herramentales = np.array(df_p.loc[:, 'Herramentales'])
    Fecha_proy = np.array(df_p.loc[:, 'Fecha_Proy'])
    Referencia = np.array(df_p.loc[:, 'Referencia'])
    Nombre = np.array(df_p.loc[:, 'Nombre'])
    ...
    for l in lista:
        ...
        df_all = pd.DataFrame()
        df_p4=pd.read_excel(data_file,sheet_name='Hoja4')
        df_all['Modelo']  = np.array(df_p4.loc[:, 'Modelo'])
        df_all['CKDp'] = np.array(df_p4.loc[:, 'CKDp'])
        df_all['CNMp'] = np.array(df_p4.loc[:, 'CNMp'])
        df_all = df_all.append({'Modelo': 'GPD155-A', 'CKDp': int(df['CKDp'].sum()), 'CNMp': int(df['CNMp'].sum())},
                               ignore_index=True)
